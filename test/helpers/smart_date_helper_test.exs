defmodule Samwise.SmartDateHelperTest do
  ...
  test "converts day to date in current month" do
    ...
    assert SmartDate.current_month_datetime(5, starting_date) == expected
  end

  test "makes a date simple" do
    date = Timex.parse!("02/23/1982", "%-m/%-d/%Y", :strftime)
    assert SmartDate.simple_date(date) == "02/23/1982"
  end
  ...
  test "shifts to next month if passed" do
    ...
    assert SmartDate.next_date(current_month_date, starting_date) == expected
  end
  ...
  test "converts day to pretty date in next month" do
    starting_date = Timex.parse!("02/23/1982", "%-m/%-d/%Y", :strftime)
    assert SmartDate.pretty_next_date(5, starting_date) == "Mar 5th"
  end
end
